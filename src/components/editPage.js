...
export const EditPage = () => {
  ...
  const { id } = useParams();
  const { pages, updatePage, language, setShowToast, showToast } =
    useContext(WikiContext);
  const page = pages.find(
    (p) => p.id === parseInt(id) && p.language === language
  );
  const availableLinks = pages
  ...
  const componentsToJsxString = (componentString) => {
    ...
    let transformed = `<><Hero title="${title.replace(
      /"/g,
      "&quot;"
    )}" subtitle="${subtitle.replace(/"/g, "&quot;")}" /> <Tags />`;
    ...
    for (const line of lines) {
      if (line.startsWith("h1:")) {
        transformed += line.replace(
          /h1:([^<]+)/,
          (_, value) => `<FirstTitle title="${value.trim()}" />`
        );
      } else if (line.startsWith("h2:")) {
        transformed += line.replace(
          /h2:([^<]+)/,
          (_, value) => `<SecondTitle title="${value.trim()}" />`
        );
        ...
        transformed += line.replace(/link:([^<]+)/, (_, value) => {
          ...
          if (!availableLinks.includes(value.trim())) {
            console.error(
              `Link "${value.trim()}" not found in available links`
            );
            setShowToast({
              mode: "error",
              message: `Link "${value.trim()}" not found in available links`,
              show: true,
            });
        ...
        transformed += line.replace(
          /p:([^<]+)/,
          (_, value) => `<Paragraph title="${value.trim()}" />`
        );
      }
    }
  ...
  const [paragraphs, setParagraphs] = useState(
    jsxStringToComponents(page?.content)
  );
  ...
  const handleSave = () => {
    if (!title || !subtitle) {
      setShowToast({
        mode: "error",
        message: "Please add title and subtitle!",
        show: true,
      });
      setTimeout(() => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className={"relative w-full"}>
          <div className="z-50 absolute bottom-2 start-0 left-1 flex items-center pointer-events-none">
            <img
              src={tag}
              className="z-50 w-6 h-6 flex self-center ml-1"
              alt="search"
            />
          </div>
          <input
            type={"text"}
            placeholder={"%page-name-example%"}
            value={newTag}
        ...
        <div className={"w-full flex align-middle justify-between mt-8"}>
          <button
            ...
            className={
              "bg-red-800 text-white border border-solid shadow-sm border-white border-opacity-30 rounded-xl px-4 py-2"
            }
          >
            Cancel
          </button>
          <button
            ...
            className={
              "bg-green-800 text-white border border-solid shadow-sm border-white border-opacity-30 rounded-xl px-4 py-2"
            }
          >
            Add new tag
          </button>
      ...
      >
        <h4 className={"font-bold mt-3 text-center"}>
      ...
      <div className="mt-6 flex flex-col px-5">
        ...
        {/* Add title  */}
        <div className="flex justify-between items-center px-3 my-3 mb-5">
          <div className="flex flex-col text-2xl font-bold text-blue-700 whitespace-nowrap  ">
            Edit {page?.title}
          </div>
          <svg width="32px" height="32px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"   onClick={() => setShowHelpModal(!showHelpModal)}><path d="M12 4C9.243 4 7 6.243 7 9h2c0-1.654 1.346-3 3-3s3 1.346 3 3c0 1.069-.454 1.465-1.481 2.255-.382.294-.813.626-1.226 1.038C10.981 13.604 10.995 14.897 11 15v2h2v-2.009c0-.024.023-.601.707-1.284.32-.32.682-.598 1.031-.867C15.798 12.024 17 11.1 17 9c0-2.757-2.243-5-5-5zm-1 14h2v2h-2z" fill="#1d4ed8"/></svg>
        </div>
        ...
        <div className="relative">
          <input
            type="text"
            className="block w-full pl-2 py-3 text-base text-gray-900 placeholder-gray-500 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
            placeholder="Title"
            value={title}
        ...
        <div className="flex gap-3 self-start mt-4 text-sm font-medium tracking-wide text-center text-white whitespace-nowrap">
          {tags &&
            tags.map((tag) => (
              <div
                className="justify-center px-4 py-2.5 bg-sky-500 rounded-3xl"
                key={tag}
              >
                {tag}
                <br />
        ...
        <div className="flex gap-5 justify-between px-px mt-20 w-full text-base font-semibold text-center text-neutral-100">
          <div
            className="justify-center px-9 py-3 whitespace-nowrap bg-red-600 rounded-xl border border-solid shadow-sm border-white border-opacity-30"
            onClick={() => navigate(-1)}
          >
            Cancel
          </div>
          <div
            className="justify-center px-4 py-2.5 bg-green-800 rounded-xl border border-solid shadow-sm border-white border-opacity-30"
      ...
      {modalInfo && (
        <div
          id="extralarge-modal"
          tabIndex="-1"
          className="fixed top-40 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full"
        >
          ...
          <div className="relative w-full max-w-7xl max-h-full">
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 ">
              ...
              <div className="p-4 md:p-5 space-y-4 dark:text-white">
                ...
                <h3 className="text-lg font-semibold">
                    Decide Title and Subtitle   </h3>
                   This will be used in the Hero 
                 

                <h3 className="text-lg font-semibold">
                  Write the content
                </h3>
                Write following the markup, you can see the existent pages as example or click the info icon on the content to see the markup sintax

                {/* list with bullet points all the available link */}
                <h3 className="text-lg font-semibold">Additional Infos</h3>
                After creating the page, the name of the new page can be linked to the rest of the pages by using it as a link!
              </div>
            </div>
          </div>
        </div>
